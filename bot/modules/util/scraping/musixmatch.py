...
class VerificationError(Exception):
    def __init__(self) -> None:
        super().__init__("MusixMatch has responded with human verification, and there is no bypasser, atleast yet.")
...
class MusixMatchScraper:
    ...
    async def search(
        ...
        for track in tracks:
            ...
            title_tag = track.find("h2", "media-card-title") \
                .find("a", class_="title")
            ...
            artist_container = track.find("h3", class_="media-card-subtitle") \
                .find("a")
            artist_url = self._build_url(artist_container["href"])
            artist_name = artist_container.text
            artist = Artist(artist_name, artist_url)
            
            album_cover = track.find("div", "media-card-picture") \
                .find("img") \
                .get("srcset")
            album_cover_variations = re.findall(
                self._search_album_cover_regex,
                album_cover
            )
            best_album_cover = album_cover_variations[-1] if album_cover_variations else None
    ...
    async def get_song(self, url: str) -> Song:
        ...
        album_cover = soup.find("div", class_="banner-album-image-desktop") \
            .find("img") \
            .get("src")
